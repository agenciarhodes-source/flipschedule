...
export const platformPermissions = [
  ...
  "platform.tenants.manage_status",
  "platform.users.read",
  "platform.users.manage_status",
...
export function canManagePlatformOperator(
  actor: PlatformOperatorRole,
  target: PlatformOperatorRole,
  nextRole?: PlatformOperatorRole,
) {
  if (actor === "PLATFORM_OWNER") return true;
  return (
    actor === "PLATFORM_ADMIN" &&
    target !== "PLATFORM_OWNER" &&
    nextRole !== "PLATFORM_OWNER"
  );
}

export function ensureLastPlatformOwner(input: {
  targetRole: PlatformOperatorRole;
  targetStatus: string;
  nextRole?: PlatformOperatorRole;
  nextStatus?: string;
  ...
  const removes =
    ...
    ((input.nextRole && input.nextRole !== "PLATFORM_OWNER") ||
      (input.nextStatus && input.nextStatus !== "ACTIVE"));
  if (removes && input.activeOwnerCount <= 1) {
