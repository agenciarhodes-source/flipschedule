...
import {
  ...
  lockCommercialQuota,
} from "./commercial-plan-quota";

const editableRole = z.enum([
  "MANAGER",
  "RECEPTIONIST",
  "PROFESSIONAL",
  "AGENCY_LEAD",
...
const quotaFailure = () =>
  actionFailure(
    "CONFLICT",
    "O plano atual atingiu o limite de usuários. Revogue um convite ou acesso, ou altere o plano para liberar uma nova vaga.",
  );
...
export class TeamService {
  ...
  private allows(permission: Permission) {
    return hasPermission(this.context.membershipRole, permission);
  }
  ...
  async invite(input: unknown): Promise<ActionResult<{ id: string; url: string; expiresAt: string }>> {
    ...
    const parsed = z
      .object({
        ...
        role: editableRole,
        clinicIds: z.array(idSchema).max(100).default([]),
      })
      .safeParse(input);
    if (!parsed.success) {
      return actionFailure("VALIDATION_ERROR", "Revise o e-mail, o papel e as unidades informadas.");
    }
    const clinicIds = [...new Set(parsed.data.clinicIds)];
    if (!tenantWideRole(parsed.data.role) && clinicIds.length === 0) {
      return actionFailure("VALIDATION_ERROR", "Selecione ao menos uma unidade para este papel.");
    }
    ...
    try {
      const invitation = await this.prisma.$transaction(
        async (tx) => {
          ...
          if (!tenantWideRole(parsed.data.role) && clinicCount !== clinicIds.length) {
            throw new Error("CLINIC_SCOPE_INVALID");
          }
          await assertUserQuotaAvailable(tx, this.context.tenantId, "ADD_RESERVATION");
          ...
          if (!tenantWideRole(parsed.data.role)) {
            await tx.tenantInvitationClinicAccess.createMany({
          ...
          await this.audit(tx, "team.invitation.created", "TenantInvitation", row.id, {
            role: parsed.data.role,
            clinicCount: tenantWideRole(parsed.data.role) ? null : clinicIds.length,
            quotaReserved: true,
          });
          return row;
        },
      ...
      if (error instanceof Error && error.message === "CLINIC_SCOPE_INVALID") {
        return actionFailure("VALIDATION_ERROR", "Uma das unidades selecionadas não está disponível.");
      }
      if (error instanceof Error && error.message === "COMMERCIAL_USER_LIMIT_REACHED") {
        return quotaFailure();
      }
    ...
    try {
      const row = await this.prisma.$transaction(
        async (tx) => {
          ...
          const wasExpired = current.expiresAt <= new Date();
          if (wasExpired) {
            await assertUserQuotaAvailable(tx, this.context.tenantId, "ADD_RESERVATION");
          }
      ...
      return actionFailure(
        error instanceof Error && error.message === "NOT_FOUND" ? "NOT_FOUND" : "UNAVAILABLE",
        "Não foi possível rotacionar o convite.",
      );
    }
  }

  async revokeInvitation(id: string) {
    return this.invitationMutation(id, "team.revoke", "team.invitation.revoked", {
      revokedAt: new Date(),
    });
  }

  private async invitationMutation(
    id: string,
  ...
  async updateRole(id: string, role: unknown) {
    ...
    if (!parsed.success || !idSchema.safeParse(id).success) {
      return actionFailure("VALIDATION_ERROR", "Papel inválido.");
    }
  ...
  private async changeMember(
    id: string,
    action: string,
    data: { role?: MembershipRole; status?: "ACTIVE" | "SUSPENDED" | "REVOKED" },
  ) {
    if (!idSchema.safeParse(id).success) {
      return actionFailure("VALIDATION_ERROR", "Membro inválido.");
    }
    try {
      return await this.prisma.$transaction(
        async (tx) => {
          ...
          if (
            target.role === "OWNER" &&
            (data.role || (data.status && data.status !== "ACTIVE"))
          ) {
            ...
            if (owners <= 1) {
              return actionFailure(
                "CONFLICT",
                "A organização deve manter ao menos um proprietário ativo.",
              );
            }
          }
          if (data.status === "ACTIVE" && target.status === "REVOKED") {
            await assertUserQuotaAvailable(tx, this.context.tenantId, "ADD_RESERVATION");
          }
          await tx.membership.update({ where: { id }, data });
          await this.audit(tx, action, "Membership", id, {
            ...data,
            quotaReserved: data.status === "ACTIVE" && target.status === "REVOKED",
          });
          return { ok: true as const, data: { id } };
        },
        { isolationLevel: "Serializable" },
  ...
  async transferOwnership(targetId: string, confirmation: unknown) {
    ...
    if (!parsed.success || !idSchema.safeParse(targetId).success) {
      return actionFailure("VALIDATION_ERROR", "Confirmação inválida.");
    }
    try {
      return await this.prisma.$transaction(
        async (tx) => {
          ...
          if (!tenant || parsed.data !== tenant.name) {
            return actionFailure(
              "VALIDATION_ERROR",
              "Digite exatamente o nome da organização para confirmar.",
            );
          }
          const target = await tx.membership.findFirst({
          ...
          if (!target || target.id === this.context.membershipId) {
            return actionFailure("CONFLICT", "Selecione outro membro ativo.");
          }
...
export class PublicInvitationService {
  ...
  async inspect(token: string) {
    ...
    return { ...row, emailMasked: `${local?.slice(0, 1) ?? "*"}***@${domain}` };
  }

  async accept(token: string, user: { id: string; email: string }) {
    if (token.length < 32) {
      return actionFailure("NOT_FOUND", "Convite inválido ou expirado.");
    }
    try {
      return await this.prisma.$transaction(
        async (tx) => {
          ...
          if (normalizeEmail(user.email) !== invite.emailNormalized) {
            return actionFailure("ACCESS_DENIED", "Entre com a conta correspondente ao convite.");
          }

          await lockCommercialQuota(tx, invite.tenantId);
          await assertUserQuotaAvailable(tx, invite.tenantId, "CURRENT_RESERVATIONS_VALID");
          ...
          if (existing?.status === "ACTIVE") {
            return actionFailure("CONFLICT", "Esta conta já faz parte da organização.");
          }
          const membership = existing
            ? await tx.membership.update({
                where: { id: existing.id },
                data: { role: invite.role, status: "ACTIVE", acceptedAt: new Date() },
              })
          ...
          if (!tenantWideRole(invite.role)) {
            const scopes = await tx.tenantInvitationClinicAccess.findMany({
              where: { tenantId: invite.tenantId, invitationId: invite.id },
              select: { clinicId: true },
            });
            if (scopes.length === 0) {
              return actionFailure(
                "CONFLICT",
                "O convite não possui unidades disponíveis. Solicite um novo convite.",
              );
            }
            await tx.membershipClinicAccess.updateMany({
              where: { tenantId: invite.tenantId, membershipId: membership.id, active: true },
              data: { active: false },
            });
            await tx.membershipClinicAccess.createMany({
              data: scopes.map(({ clinicId }) => ({
                tenantId: invite.tenantId,
          ...
          await tx.auditLog.create({
            data: {
              ...
              outcome: "SUCCESS",
              metadata: {
                clinicScoped: !tenantWideRole(invite.role),
                quotaValidated: true,
              },
            },
          });
          return {
            ok: true as const,
            data: { membershipId: membership.id, tenantSlug: invite.tenant.slug },
          };
        },
        { isolationLevel: "Serializable" },
      );
      ...
      return actionFailure("UNAVAILABLE", "Não foi possível aceitar o convite.");
    }
  }
}
