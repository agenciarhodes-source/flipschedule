...
export class AsaasHttpClient {
  ...
  private get baseUrl() {
    return this.environment === "production" ? ASAAS_PRODUCTION_BASE_URL : ASAAS_SANDBOX_BASE_URL;
  }
  ...
  async request<T>(
    ...
    if (!path.startsWith("/") || path.startsWith("//")) {
      throw new BillingPermanentError("ASAAS_PATH_INVALID");
    }
    ...
    try {
      const response = await (this.options.fetch ?? fetch)(`${this.baseUrl}${path}`, {
        ...
        headers: {
          ...
          "User-Agent": `FlipSchedule/${this.environment}`,
      ...
      if (
        ...
        error instanceof BillingTemporaryError ||
        error instanceof BillingPermanentError
      ) {
        throw error;
      }
      throw new BillingTemporaryError();
...
export function validateHostedCheckoutUrl(value: string) {
  ...
  if (
    ...
    throw new BillingPermanentError("INVALID_CHECKOUT_URL");
  }
  return url.toString();
}
