...
export class AsaasBillingAdapter implements BillingProviderAdapter {
  ...
  async createRecurringCheckout(request:RecurringCheckoutRequest):Promise<HostedCheckout>{
    ...
    const data=await this.http.request<Json>("POST","/checkouts",{chargeTypes:["RECURRENT"],billingTypes:request.plan.allowedBillingTypes,minutesToExpire:this.checkoutExpirationMinutes,externalReference:request.externalReference,items:[{name:request.plan.displayName,quantity:1,value:request.plan.priceCents/100}],subscription:{cycle:request.plan.cycle,nextDueDate:request.nextDueDate},callback:request.callback,...(request.customerData?{customerData:request.customerData}:{})},request.correlationId);
