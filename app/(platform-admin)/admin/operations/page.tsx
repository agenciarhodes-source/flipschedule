      ...
      <section>
        ...
        <div className="mt-4 space-y-3">
          {data.webhooks.length === 0 ? <p className="card-surface p-5 text-sm text-ink-muted">Nenhum webhook corresponde aos filtros.</p> : data.webhooks.map((row) => (
            <article className="card-surface p-5" key={row.id}>
              <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-medium">{row.tenant.name} · {row.provider}</p><p className="font-mono text-xs text-ink-muted">/{row.tenant.slug} · {row.id}</p></div><span className="rounded-full border border-line px-3 py-1 text-xs font-semibold">{statusLabel[row.status] ?? row.status}</span></div>
