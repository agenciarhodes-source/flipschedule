...
async function main(){const prisma=createCliPrismaClient();try{const result=await verifyExternalStaging(prisma);console.info(JSON.stringify(result));if(result.errorCodes.length)process.exitCode=1}finally{await prisma.$disconnect()}}
