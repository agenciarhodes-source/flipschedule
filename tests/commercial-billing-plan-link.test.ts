...
describe("commercial billing plan link", () => {
  it("resolves the canonical commercial plan by exact code", async () => {
    ...
    await expect(resolveCommercialPlanLink(db, "PRO")).resolves.toEqual({
      id: "plan-1",
      code: "PRO",
    });
  ...
  it("fails closed when a provider subscription references an unknown plan", async () => {
    const db = {
      commercialPlan: { findUnique: vi.fn().mockResolvedValue(null) },
    } as unknown as PrismaClient;

    await expect(resolveCommercialPlanLink(db, "UNKNOWN")).rejects.toMatchObject({
  ...
  it("binds newly materialized Asaas subscriptions before quota enforcement", () => {
    ...
    expect(runtime).toContain("resolveCommercialPlanLink(tx,checkout.planCode)");
  ...
  it("repairs legacy unlinked subscriptions without overwriting an existing link", () => {
    const runtime = readFileSync(
      "domains/infrastructure/integrations/async-runtime.ts",
      "utf8",
    );
    ...
    expect(runtime).toContain(
      "existing.commercialPlanId?null:await resolveCommercialPlanLink(tx,existing.planCode)",
    );
    expect(reconciliation).toContain(
      "local.commercialPlanId?null:await resolveCommercialPlanLink(this.prisma,local.planCode)",
    );
